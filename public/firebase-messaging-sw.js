...
self.addEventListener('notificationclick', (event) => {
  ...
  const url = notificationUrlBuilder.constructUrl(data.entityTable, data.notificationType);
  ...
  event.waitUntil(
    clients.matchAll({ type: "window" }).then((clientsArr) => {
      ...
      const alreadyFocusedWindow = clientsArr.find((windowClient) =>
        windowClient.url === url && windowClient.focus()
      );
